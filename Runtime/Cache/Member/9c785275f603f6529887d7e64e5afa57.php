...
<script type="text/javascript">
		...
		<div class="zh_cont_r">
			<h3>当前位置 > <span style="color: #D41010;">申请记录</span></h3>
			<div class="r_main">
				<div class="mes_con">
					<div class="tabtil">
					<ul>
						<li id="lab_1" class="on"><a>全部申请</a></li>
						...
						<div class="biaoge">
							<table class="bordered">
								...
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$chkdata): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><?php if($chkdata["status"] == '0'): ?>审核中 
										<?php elseif($chkdata["status"] == 1): ?>已通过<a href="<?php echo U('Home/Borrow/index');?>" style="color:red" title="点击借款"><br>点击借款</a>
										<?php elseif($chkdata["status"] == 2): ?> 申请不通过 
										<?php elseif($chkdata["status"] == 3): ?> 申请已完成<?php endif; ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
								</tr>
								<tr>
