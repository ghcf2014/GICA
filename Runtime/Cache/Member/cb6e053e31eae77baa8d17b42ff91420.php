		...
		<div class="zh_cont_r">
			...
			<?php else: ?>
				<div class="lg-confirml" >
					<p class="i-btn-typical" id="i-btn-typical">您有未完成的认证。</p>
				</div>
				<div class="lg-confirm" >
					<a data-spm="d4919529" target="_blank" class="i-btn-typical" title="<?php echo ($tasks["task_detail"]); ?>" href="/Member/Userinfo/<?php echo ($tasks["main"]); ?>"><?php echo ($tasks["task_detail"]); ?></a>
				</div><?php endif; ?>
			</div>
			<!-- 账户详情 -->
			<div class="zh_cont_r_con_bot">
				<h4 ><img src="/Public/Home/images/img/iconfont-icon.png" style="border-radius: 100px;"width="32" height="32">账户详情：</h4>
				<div class="zh_cont_r_bot">
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul>
						<li>账户总额（元）：<?php echo ($vo['account_money']+$vo['money_freeze']); ?></li>
						<li>可用余额（元）：<?php echo ((isset($vo["account_money"]) && ($vo["account_money"] !== ""))?($vo["account_money"]):"0.00"); ?></li>
						<li>冻结金额（元）：<?php echo ((isset($vo["money_freeze"]) && ($vo["money_freeze"] !== ""))?($vo["money_freeze"]):"0.00"); ?></li>
					</ul><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
				<!-- 投资总汇 -->
				<!-- 借款总汇 -->
				<h4><img src="/Public/Home/images/img/iconfont-lijin.png" style="border-radius: 100px;"width="32" height="32">收益统计：</h4>
				<div class="zh_cont_r_bot">
					<ul>
						<li>已收总收益（元）：<?php echo ($bm['interest']+$bm['reward']+$bm['oter_reward']); ?></li>
						<li>已收投资收益（元）：<?php echo ((isset($bm["interest"]) && ($bm["interest"] !== ""))?($bm["interest"]):"0.00"); ?></li>
						<li>已收奖励（元）：<?php echo ((isset($bm["reward"]) && ($bm["reward"] !== ""))?($bm["reward"]):"0.00"); ?></li>
						<li>其他收益（元）：<?php echo ((isset($bm["oter_reward"]) && ($bm["oter_reward"] !== ""))?($bm["oter_reward"]):"0.00"); ?></li>
						<li>推广提成收益（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
					</ul>
				</div>
				<h4><img src="/Public/Home/images/img/iconfont-jianzhi.png" style="border-radius: 100px;"width="32" height="32">待收统计：</h4>
				<div class="zh_cont_r_bot">
				    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul>
							<li>待收本息（元）：<?php echo ($vo['money_collect']+$interest); ?></li>
							<li>待收本金（元）：<?php echo ((isset($vo['money_collect']) && ($vo['money_collect'] !== ""))?($vo['money_collect']):"0.00"); ?></li>
							<li>待收利息（元）：<?php echo ((isset($interest) && ($interest !== ""))?($interest):"0.00"); ?></li>
						</ul><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
				<h4><img src="/Public/Home/images/img/iconfont-zhuanzhang.png" style="border-radius: 100px;"width="32" height="32">充值提现：</h4>
				<div class="zh_cont_r_bot">
				<?php if(is_array($paymoney)): $i = 0; $__LIST__ = $paymoney;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul>
						<li>充值总金额（元）：<?php echo ((isset($vo["money"]) && ($vo["money"] !== ""))?($vo["money"]):"0.00"); ?></li>
