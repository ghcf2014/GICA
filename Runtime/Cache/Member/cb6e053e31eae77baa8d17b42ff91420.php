...
<script type="text/javascript">
		...
		<div class="zh_cont_r">
			...
			<dl style="padding-bottom: 10px;">
				<dt>
					<div class="img">
						<?php if(is_array($list3)): $i = 0; $__LIST__ = $list3;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["logo_url"] == ''): ?><img
							src="/Public/Home/images/default-img.jpg" width="200"
							height="200"> <?php elseif($vo["logo_url"] != ''): ?>
						<img src="Uploads/User/<?php echo ($vo["logo_url"]); ?>" width="200" height="200" /><?php endif; endforeach; endif; else: echo "" ;endif; ?>
						<p>
							<a id="test2">更换图像</a>
						</p>
					</div>
				...
				<dd>
					<p>
						用户名称：<span style="color: #66ccfb;"><?php echo get_username();?></span>
					</p>
					<p>
						会员积分：<span><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; echo ($vo["score"]); ?>分<?php endforeach; endif; else: echo "" ;endif; ?></span>
					</p>
					<p>
						注册时间：<span><?php echo (date('Y-m-d',$vo["reg_time"])); ?></span>
					</p>
					<p>
						所属客服：<span>客服-丫丫</span>
					</p>
					<p>
						登录时间：<span><?php echo (date('Y-m-d H:i:s',$vo["last_login_time"])); ?></span>
					</p>
					<p>
						站内信：<span>（<a href="<?php echo U('System/usermailindex');?>" style="text-decoration: none;"><font
								color="#66ccfb;"><?php echo is_sysmsg();?></font></a>）
						</span>条
					</p>
					<p>登录IP:<span><?php echo get_client_ip();?></span></p>
					<p><a href="javascript:;">充值</a><a href="javascript:;">提现</a><a href="javascript:;">增值服务</a></p>
			...
			<ul class="ul01">
				...
				<?php if(is_array($mstatus)): $i = 0; $__LIST__ = $mstatus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ms): $mod = ($i % 2 );++$i;?><li>认证管理：</li>
				<li>
					<?php if($ms["email_status"] == 1): ?><a href="<?php echo U('Userinfo/usermailbanding');?>"><img src="/Public/Home/images/img/l_yx_pic.jpg" title="邮箱已认证" /> </a> 
					<?php else: ?> 
						<a href="<?php echo U('Userinfo/usermailbanding');?>"><img src="/Public/Home/images/img/h_yx_pic.jpg" title="邮箱未认证" /> </a><?php endif; ?>
				</li>
				<li>
					<?php if($ms["phone_status"] == 1): ?><a href="<?php echo U('/Member/Userinfo/userfindpassbyphone');?>"><img src="/Public/Home/images/img/l_sj_pic.jpg" title="手机已认证" /></a>

					<?php else: ?>
						<a href="<?php echo U('/Member/Userinfo/userfindpassbyphone');?>"><img src="/Public/Home/images/img/h_sj_pic.jpg" title="手机未认证" /></a><?php endif; ?>

				</li>
				<li>
					<?php if($ms["id_status"] == 1): ?><a href="<?php echo U('Userinfo/userselfset');?>"><img src="/Public/Home/images/img/l_sfz_pic.jpg" title="身份证已认证" /> </a> 
					<?php else: ?>
						 <a href="<?php echo U('Userinfo/userselfset');?>"><img src="/Public/Home/images/img/h_sfz_pic.jpg" title="身份证未认证" /> </a><?php endif; ?>
				</li>
				<li>
					<?php if($ms["credit_status"] == 1): ?><a href="<?php echo U('Userinfo/userbankset');?>"> <img src="/Public/Home/images/img/l_yhk_pic.jpg" title="银行卡已认证" /> </a> 
					<?php else: ?> 
						<a href="<?php echo U('Userinfo/userbankset');?>"> <img src="/Public/Home/images/img/h_yhk_pic.jpg" title="银行卡未认证" /> </a><?php endif; ?>
				</li>
				<li><?php if($ms["video_status"] == 1): ?><a href="<?php echo U('Userinfo/userrenewalvip');?>"> <img src="/Public/Home/images/img/l_vip_pic.jpg" title="VIP已认证" /> </a>
					<?php else: ?>
						<a href="<?php echo U('Userinfo/userrenewalvip');?>"> <img src="/Public/Home/images/img/h_vip_pic.jpg" title="VIP未认证" /> </a><?php endif; ?>
			...
			<div class="zh_cont_r_con_bot">
				...
				<div class="zh_cont_r_bot">
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul>
						<li>账户总额：<?php echo ((isset($vo["account_money"]) && ($vo["account_money"] !== ""))?($vo["account_money"]):"0.00"); ?></li>
						<li>可用余额：<?php echo ((isset($vo["account_money"]) && ($vo["account_money"] !== ""))?($vo["account_money"]):"0.00"); ?></li>
						<li>冻结金额：<?php echo ((isset($vo["account_money"]) && ($vo["account_money"] !== ""))?($vo["account_money"]):"0.00"); ?></li>
				...
				<div class="zh_cont_r_bot">
					<ul>
						<li>已收总收益（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
						<li>已收收益（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
						<li>已收奖励（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
						<li>其他收益（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
						<li>推广提成收益（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
					</ul>
				</div>
				<h4>待收统计：</h4>
				<div class="zh_cont_r_bot">
				    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul>
							<li>待收本息（元）：<?php echo ((isset($vo['money_collect']) && ($vo['money_collect'] !== ""))?($vo['money_collect']):"0.00"); ?></li>
							<li>待收本金（元）：<?php echo ((isset($vo['money_collect']) && ($vo['money_collect'] !== ""))?($vo['money_collect']):"0.00"); ?></li>
							<li>待收利息（元）：<?php echo ((isset($vo['money_collect']) && ($vo['money_collect'] !== ""))?($vo['money_collect']):"0.00"); ?></li>
						</ul><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
				<h4>充值提现：</h4>
				<div class="zh_cont_r_bot">
					<ul>
						<li>充值总金额（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
						<li>提现总金额（元）：<?php echo ((isset($bm["borrow_money"]) && ($bm["borrow_money"] !== ""))?($bm["borrow_money"]):"0.00"); ?></li>
					</ul>
				</div>
			</div>
					...
					function() {
						...
						$
								.layer({
									...
									area : [ '260px', '100px' ],
									page : {
										html : '<form action="/index.php?s=/Member/Index/upload" enctype="multipart/form-data" method="post"><input type="file"  name="filename" value=""><input  type="submit"  value="上传"></from>'
									},
								});
					});
</script> 
