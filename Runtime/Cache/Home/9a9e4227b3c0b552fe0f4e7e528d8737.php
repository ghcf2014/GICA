             ...
             <div class="newlist"> 
               <ul>
                  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><li>
                      <span><?php echo (get_username($data["uid"])); ?>发表于<?php echo (date('Y-m-d H:i:s',$data["create_time"])); ?></span>
                      <a href="frontNewsDetails.htm?id=${bean.id}"><a href="<?php echo U('Article/detail?id='.$data['id']);?>"><?php echo ($data["title"]); ?></a></a>
