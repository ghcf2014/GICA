		...
		<div class="nymain">
			<div class="lcnav">
				<div class="tab">
					<div class="tabmain">
						<ul>
							<li class="on" style="padding: 0 40px;color: #66ccfb;background: url(/Public/Home/images/gg.png) no-repeat;">
								公告详情
							</li>
						</ul>
					</div>
			...
			<div class="lcmain">
				<div class="lcmain_l" style="width: 675px">
					<div class="lctab" style="padding: 0 10px;">
						<div class="gginfo" style="padding-bottom: 30px">
							...
							<p class="time">
								发布时间：<?php echo (date('Y-m-d H:i',$info["create_time"])); ?> 点击次数：<?php echo ($info["view"]); ?>
							</p>
							<div id="imgxz">
								<section id="contents" style="color: #333;"><?php echo ($info["content"]); ?></section>
							</div>
						</div>
						<div class="sxnews">
							<ul>
								<li style="background: none; line-height: 30px">
                    <?php $prev = D('Document')->prev($info); if(!empty($prev)): ?><a href="<?php echo U('?id='.$prev['id']);?>">上一篇： </a><a href="<?php echo U('?id='.$prev['id']);?>"><?php echo (msubstr($prev['title'],0,18)); ?></a><?php endif; ?>
								</li>
								<li style="background: none; line-height: 30px">
                    <?php $next = D('Document')->next($info); if(!empty($next)): ?><a href="<?php echo U('?id='.$next['id']);?>">下一篇：</a><a href="<?php echo U('?id='.$next['id']);?>"><?php echo (msubstr($next['title'],0,18)); ?></a><?php endif; ?>
								</li>
							</ul>
						</div>
					</div>
				...
				<div class="lbox">
                   <h2 style="color: #66ccfb;background: url(/Public/Home/images/xw.png) no-repeat left 3px;padding-left: 40px;border-bottom: 1px solid #ddd;">网站动态</h2>
                   <div class="dongtai" style="padding-bottom:30px">
                   <ul><?php $__CATE__ = D('Category')->getChildrenId(1);$__LIST__ = D('Document')->page(!empty($_GET["p"])?$_GET["p"]:1,10)->lists($__CATE__, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('Article/detail?id='.$list['id']);?>"><?php echo ($list["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
           	       </ul>
                </div>
                </div>
				</div>
